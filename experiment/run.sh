...
# ---------------------------------------------------- --show-prompts mode --
# Emits, as markdown, the exact prompt every arm sends (rendered for the
# first ticket, with room/token placeholders). Generated from the same
# functions the experiment runs, so the published document cannot drift
# from the harness: experiment/run.sh --show-prompts > experiment/PROMPTS.md
if [ -n "${SHOW_PROMPTS:-}" ]; then
  t=$(jq -c '.[0]' experiment/tickets.json)
...
HDR
  for arm in $ARMS; do
    ...
    printf '## arm: %s\n\n```text\n' "$arm"
    prompt_for "$subject" "$context" "$block" "ROOM_ID" "PARTICIPANT_TOKEN"
    printf '```\n\n'
  done
  cat <<'TXT'
...
```text
TXT
  prompt_text_for "$subject" "$context" "$(arm_block low 2)"
  printf '```\n'
  exit 0
fi
